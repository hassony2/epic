...
def soft_feature_blending(
    colors, fragments, blend_params=None, znear: float = 0.01, zfar: float = 10
) -> torch.Tensor:
    """
    Returns:
    ...
    # colored = (1 - torch.prod(1 - (prob_map * z_prob).unsqueeze(-1) * colors, dim=-2))
    # colored = (1 - torch.prod(1 - (prob_map).unsqueeze(-1) * colors, dim=-2))
    colored = 1 - torch.prod(
        1 - (prob_map * z_prob).unsqueeze(-1) * colors, dim=-2
    )
