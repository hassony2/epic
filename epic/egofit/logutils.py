...
COLORS = (
    palettes.Colorblind[max(palettes.Colorblind)]
...
def make_compare_plots(
    plots, local_folder, tools="pan,wheel_zoom,box_zoom,reset,save"
):
    bokeh_figs = []
...
def plotvals2bokeh(
    plot_vals, local_folder, tools="pan,wheel_zoom,box_zoom,reset,save"
):
...
def df2html(df, local_folder="", drop_redundant=True):
    ...
    for key in keys:
        if "img_path" in key:
            ...
            format_dicts[key] = partial(
                plotvals2bokeh, local_folder=local_folder
            )
