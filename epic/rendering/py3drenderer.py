import torch
from cosypose.rendering import py3drendutils
...
class Renderer:
    ...
    def render_batch(self, obj_infos, cam_infos):
        ...
        verts = torch.bmm(TCO[:, :3, :3], verts.permute(0, 2, 1)).permute(0, 2, 1)
        verts = verts + TCO[:, :3, 3].unsqueeze(1)
        image_sizes = cam_infos["resolution"]
        images = py3drendutils.batch_render(verts, faces, K=K, image_sizes=image_sizes)
        return {"rgb": images}
