# Stolen from
# https://github.com/kornia/kornia/blob/
# e18623e8b9e2693d107e66dabb23c44fcfebd1ab/kornia/geometry/conversions.py
import torch
...
def angle_axis_to_rotation_matrix(
    angle_axis: torch.Tensor, eps: float = 1e-8
) -> torch.Tensor:
...
def rotation_matrix_to_angle_axis(
    ...
    if not isinstance(rotation_matrix, torch.Tensor):
        raise TypeError(
            "Input type is not a torch.Tensor. Got {}".format(
...
def rotation_matrix_to_quaternion(
    rotation_matrix: torch.Tensor, eps: float = 1e-6
) -> torch.Tensor:
    ...
    def safe_zero_division(
        ...
        eps: float = 0.000001  # type: ignore
    ...
    quaternion: torch.Tensor = torch.where(
        trace > 0.0, trace_positive_cond(), where_1
    )
...
def quaternion_to_rotation_matrix(quaternion: torch.Tensor) -> torch.Tensor:
    ...
    if not bool(torch.norm(quaternion[..., :3]) == 1.0):
...
def quaternion_to_angle_axis(
    quaternion: torch.Tensor, eps: float = 1e-5
) -> torch.Tensor:
    ...
    xyz = quaternion[..., :3]
    w = quaternion[..., -1:]
    norm = torch.norm(xyz, dim=-1).unsqueeze(-1)
...
def quaternion_log_to_exp(
    quaternion: torch.Tensor, eps: float = 1e-8
) -> torch.Tensor:
    ...
    norm_q: torch.Tensor = torch.norm(
        quaternion, p=2, dim=-1, keepdim=True
    ).clamp(min=eps)
...
def angle_axis_to_quaternion(angle_axis: torch.Tensor) -> torch.Tensor:
    ...
    quaternion: torch.Tensor = torch.where(
        theta_squared > 0.0, cond_1(), cond_2()
    )
    return quaternion
...
def normalize_pixel_coordinates3d(
    ...
    if pixel_coordinates.shape[-1] != 3:
        raise ValueError(
            "Input pixel_coordinates must be of shape (*, 3). "
            "Got {}".format(pixel_coordinates.shape)
        )
    # compute normalization factor
    dhw: torch.Tensor = (
