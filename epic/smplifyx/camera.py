...
PerspParams = namedtuple(
    "ModelOutput", ["rotation", "translation", "center", "focal_length"]
)
...
class PerspectiveCamera(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("focal_length_x", torch.Tensor([focal_length_x]))
        self.register_buffer("focal_length_y", torch.Tensor([focal_length_y]))
    ...
    def forward(self, points):
        ...
        with torch.no_grad():
            camera_mat = torch.zeros(
                [batch_size, 2, 2], dtype=self.dtype, device=points.device
            )
    ...
    def get_camintr(self):
        camintr = torch.eye(3)
        camintr[0, 0] = self.focal_length_x
        camintr[1, 1] = self.focal_length_y
        camintr[:2, 2] = self.center
        return camintr
