...
from epic.boxutils import epic_box_to_norm, get_center_scale, get_closest, extend_props
from epic.displayutils import add_load_img
from epic import tarutils
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epic_root",
        default=
        "/mnt/disks/myssd/datasets/epic/frames/epictmp/frames_rgb_flow/rgb/train/"
    )
    ...
    with open(f'EPIC_{args.split}_object_action_correspondence.pkl',
              'rb') as p_f:
    ...
    for rec_path in sorted(rec_files):
        ...
        for rec_result in tqdm(rec_results):
            ...
            if box_props is None or video_full_id != last_video_id:
                ...
                video_data = object_data[object_data['video_id'] ==
                                         video_full_id]
                video_corresp_data = corresp_data[(
                    corresp_data['video_id'] == video_full_id)]
                object2action_frame = dict(
                    zip(video_corresp_data['object_frame'],
                        video_corresp_data.index))
                action2object_frame = dict(
                    zip(video_corresp_data.index,
                        video_corresp_data['object_frame']))

                haskeys = [
                    frame in object2action_frame for frame in video_data.frame
                ]
                newkeys = [
                    object2action_frame[frame] for frame in video_data.frame
                    if frame in object2action_frame
                ]
                print(f"keeping {sum(haskeys)} out of {len(haskeys)}")
                # equi_keys = [key for key in video_data.frame if object2action_frame[key] == key]
                # haskeys = [frame in equi_keys for frame in video_data.frame] # TODO remove
                # newkeys = [object2action_frame[equi_key] for equi_key in equi_keys]
                ...
                box_props = extend_props(box_props,
            ...
            if frame_idx in box_props:
                # Get image
                if args.use_tar:
                    tar_path = os.path.join(args.epic_root, video_full_id[:3],
                                            f"{video_full_id}.tar")
                ...
                img_name = os.path.basename(
                    rec_path.replace(".pkl",
                                     f"{frame_idx:08d}.{args.img_ext}"))
                img_path = os.path.join(res_folder, img_name)
