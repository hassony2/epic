...
from epic.boxutils import interpolate_bboxes, epic_box_to_norm, get_center_scale, get_closest, extend_props
...
for row_idx, action_row in video_action_data.iterrows():
    ...
    if not len(all_props):
        ...
        # Load first image
        start_img_path = frame_template.format(args.split, args.person_id, video_full_id, start_frame)
        ...
        detect2d.visualize_bboxes(axes[0, 1], bboxes_start_norm, labels=[noun,] * len(bboxes_start))
        ...
        affine_trans, _ = get_affine_transform(center, scale, crop_res)
        add_load_img(axes[1, 2], stop_bbox_img_path, title=f'crop', transform=affine_trans, crop_res=crop_res)
        save_verb_folder = os.path.join(save_folder, verb)
        os.makedirs(save_verb_folder, exist_ok=True)
        fig.savefig(os.path.join(save_verb_folder, f'{video_full_id}_{row_idx:06d}_{noun}.png'))
